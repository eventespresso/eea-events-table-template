...
class EES_Espresso_Events_Table_Template extends EES_Shortcode
{
    ...
    public function enqueue_scripts()
    {
        ...
        if (is_readable(EVENT_ESPRESSO_UPLOAD_DIR . 'css' . DS . 'espresso_events_table_template.css')) {
            ...
            wp_register_style('espresso_events_table_template',
        ...
        wp_register_script('espresso_events_table_template',
            EE_EVENTS_TABLE_TEMPLATE_URL . 'scripts' . DS . 'espresso_events_table_template.js', array('jquery'),
            EE_EVENTS_TABLE_TEMPLATE_VERSION, true);
        // enqueue
    ...
    public function process_shortcode($attributes = array())
    {
        ...
        if ($attributes['footable'] != 'false') {
            ...
            wp_register_style('footable-' . $attributes['table_style'],
                EE_EVENTS_TABLE_TEMPLATE_URL . 'css' . DS . 'footable.' . $attributes['table_style'] . '.css');
            wp_enqueue_style('footable-' . $attributes['table_style']);
            //FooTable Scripts
            wp_register_script('footable', EE_EVENTS_TABLE_TEMPLATE_URL . 'scripts' . DS . 'footable.js',
                array('jquery'), EE_EVENTS_TABLE_TEMPLATE_VERSION, true);
            // enqueue scripts
            wp_enqueue_script('footable');
            //FooTable Sorting
            if ($attributes['table_sort'] != 'false') {
                wp_register_script('footable-sort', EE_EVENTS_TABLE_TEMPLATE_URL . 'scripts' . DS . 'footable.sort.js',
                    array('jquery'), EE_EVENTS_TABLE_TEMPLATE_VERSION, true);
                wp_enqueue_script('footable-sort');
            }
            //FooTable Striping
            if ($attributes['table_striping'] != 'false') {
                wp_register_script('footable-striping',
                    EE_EVENTS_TABLE_TEMPLATE_URL . 'scripts' . DS . 'footable.striping.js', array('jquery'),
                    EE_EVENTS_TABLE_TEMPLATE_VERSION, true);
                wp_enqueue_script('footable-striping');
            }
            //FooTable Pagination
            if ($attributes['table_paging'] != 'false') {
                wp_register_script('footable-paginate',
                    EE_EVENTS_TABLE_TEMPLATE_URL . 'scripts' . DS . 'footable.paginate.js', array('jquery'),
                    EE_EVENTS_TABLE_TEMPLATE_VERSION, true);
                wp_enqueue_script('footable-paginate');
            }
            //FooTable Filter
            if ($attributes['table_search'] != 'false') {
                wp_register_script('footable-filter',
                    EE_EVENTS_TABLE_TEMPLATE_URL . 'scripts' . DS . 'footable.filter.js', array('jquery'),
                    EE_EVENTS_TABLE_TEMPLATE_VERSION, true);
